...
const registerUser = asyncHandler(async (req, res) => {
  ...
  let coverImamgeLocalPath;
  if (
    req.files &&
    Array.isArray(req.files.coverImage) &&
    req.files.coverImage.length > 0
  )
    coverImamgeLocalPath = req.files?.coverImage[0]?.path;
  ...
  ); //by default select method select all the fields, to remove some fields from the object we need to pass the string with prefix '-' before the field name like above
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  ...
  if (!incomingRefreshToken) throw new ApiError(401, "unathurized request");
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "Avatar image updated successfullyu"));
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) new ApiError(400, "Error while uploading on avatar");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "Cover image updated successfullyu"));
});
